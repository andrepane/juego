...
const variantLabels = { order: 'Ordenar la palabra', missing: 'Completar la sílaba ausente', intruder: 'Encontrar la sílaba intrusa', correctOrder: 'Corregir el orden', memory: 'Reconstrucción con memoria', instruction: 'Ejecutar la consigna', target: 'Alcanzar un resultado', identify: 'Identificar la operación', error: 'Detectar y corregir el error', chain: 'Transformación encadenada' };
...
export function createSessionConfigurator(form, activityId) {
  ...
  function render() {
    ...
    form.innerHTML = `<fieldset><legend>Modo de uso</legend><div class="option-grid mode-grid">${Object.values(SESSION_MODES).map(mode => radio('mode', mode.id, mode.label, mode.description, config.mode === mode.id)).join('')}</div></fieldset>
      ...
      <div id="${activityId}-advanced" class="advanced-panel${advancedOpen ? '' : ' hidden'}">
       ...
       <fieldset><legend>Tipos de reto</legend><div class="choice-grid">${Object.keys(variantLabels).filter(v => manip ? ['instruction','target','identify','error','chain'].includes(v) : ['order','missing','intruder','correctOrder','memory'].includes(v)).map(v=>option('variants',v,variantLabels[v],config.activityOptions.variants.includes(v))).join('')}</div></fieldset>
       ${!manip ? `<fieldset><legend>Número de distractores</legend><div class="round-options">${[2,3,4].map(n=>radio('distractorCount',n,String(n),'',config.activityOptions.distractorCount===n)).join('')}</div></fieldset><fieldset><legend>Posición objetivo</legend><div class="choice-grid">${[['initial','Inicial'],['medial','Medial'],['final','Final']].map(([v,l])=>option('orderTargetPositions',v,l,config.activityOptions.targetPositions.includes(v))).join('')}</div></fieldset><fieldset><legend>Exposición en memoria</legend><div class="round-options">${[2,3,5].map(n=>radio('memorySeconds',n,`${n} s`,'',config.activityOptions.memorySeconds===n)).join('')}</div></fieldset>` : ''}
  ...
  function read() {
    ...
    return normalizeSessionConfig({ activityId, mode: data.get('mode'), rounds: Number(data.get('rounds')), presetId: config.presetId,
      ...
      activityOptions: { operations: values('operations'), variants: values('variants'), distractorCount: Number(data.get('distractorCount')), targetPositions: values('orderTargetPositions'), memorySeconds: Number(data.get('memorySeconds')), operationVisible: data.get('operationVisible') !== 'false', chainSteps: 2 } });
  }
...
function summary(config) {
  ...
  const parts = [`${config.rounds} rondas`, `${lengths} sílabas`, a.complexities.map(v=>labels[v]).join(', '), a.frequencies.map(v=>frequencyLabels[v]).join(', ')];
