...
const words = [
  { id:'casa', word:'casa', syllables:['ca','sa'], syllableCount:2, structure:'CV-CV', frequency:1 },
  { id:'tomate', word:'tomate', syllables:['to','ma','te'], syllableCount:3, structure:'CV-CV-CV', frequency:2 },
  { id:'cristal', word:'cristal', syllables:['cris','tal'], syllableCount:2, structure:'CCVC-CVC', frequency:3 },
  { id:'mariposa', word:'mariposa', syllables:['ma','ri','po','sa'], syllableCount:4, structure:'CV-CV-CV-CV', frequency:1 }
];

test('crea configuraciones predeterminadas serializables para ambas actividades', () => {
  for (const id of ['order-syllables','manipulate-syllables']) { const config=createDefaultSessionConfig(id); assert.equal(config.mode,'supervised'); assert.deepEqual(JSON.parse(JSON.stringify(config)),config); assert.equal(validateSessionConfig(config).valid,true); }
});
...
test('clasifica add inicial, medial y final sin confundir espacios intermedios', () => { assert.deepEqual([0,1,2,3].map(p=>classifyTargetContext('add',p,3)),['initial','medial','medial','final']); });
