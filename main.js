...
function renderManipRound(result) {
  ...
  $('#manip-base').textContent = result.baseWord; $('#manip-instruction').textContent = result.instruction; manipRefs.helper.textContent = result.helperInstruction; const showHelp = !manip.seenHelp.has(result.operation); manip.seenHelp.add(result.operation); manipRefs.helper.classList.toggle('hidden', !showHelp); manipRefs.helpToggle.setAttribute('aria-expanded', String(showHelp));
